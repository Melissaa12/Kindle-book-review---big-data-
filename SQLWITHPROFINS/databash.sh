...
sudo chmod 600 $1
ssh -o StrictHostKeyChecking=no ubuntu@$2 -i $1 'bash -s' < ./datanew.sh $3 $5 $7 $9

# echo ssh into secondary instance
ssh -o StrictHostKeyChecking=no ubuntu@$4 -i $1 'bash -s' < ./datanew.sh $3 $5 $7 $9

# echo ssh into slaves
ssh -o StrictHostKeyChecking=no ubuntu@$6 -i $1 'bash -s' < ./datanew.sh $3 $5 $7 $9
if [[ ${9} != "STOP" ]]; then 
    ssh -o StrictHostKeyChecking=no ubuntu@$8 -i $1 'bash -s' < ./datanew.sh $3 $5 $7 $9
    if [[ ${11} != "STOP" ]]; then
        ssh -o StrictHostKeyChecking=no ubuntu@${10} -i $1 'bash -s' < ./datanew.sh $3 $5 $7 $9
        
        if [[ ${12} != "STOP" ]]; then
            ssh -o StrictHostKeyChecking=no ubuntu@${12} -i $1 'bash -s' < ./datanew.sh $1 ${12} ${13}
            
            if [[ ${14} != "STOP" ]]; then
                ssh -o StrictHostKeyChecking=no ubuntu@${14} -i $1 'bash -s' < ./datanew.sh $1 ${14} ${15}

                if [[ ${16} != "STOP" ]]; then
                    ssh -o StrictHostKeyChecking=no ubuntu@${16} -i $1 'bash -s' < ./datanew.sh $1 ${16} ${17}
                    
                    if [[ ${18} != "STOP" ]]; then
                        ssh -o StrictHostKeyChecking=no ubuntu@${18} -i $1 'bash -s' < ./datanew.sh $1 ${18} ${19}
                        
                        if [[ ${20} != "STOP" ]]; then
                            ssh -o StrictHostKeyChecking=no ubuntu@${20} -i $1 'bash -s' < ./datanew.sh $1 ${20} ${21}
                            fi
                    fi
                fi        
            fi
